  ...
  unit = 'college';
      ...
      case 'fees': {
        ...
        if (req.method === 'POST') {
          ...
          // AUTO UPDATE BALANCE - add fee to cash account
          if (+amount > 0 && u) {
            try {
              await sql`UPDATE accounts SET balance = balance + ${+amount}, updated_at = NOW() WHERE unit = ${u} AND account_type = 'Cash'`;
            } catch(e) { console.error('Balance update error:', e.message); }
          }
          return res.status(201).json(r[0]);
        }
        ...
        if (req.method === 'DELETE') {
          ...
          try {
            const fee = await sql`SELECT * FROM fee_payments WHERE id=${id} LIMIT 1`;
            if (fee.length && +fee[0].amount > 0 && fee[0].unit) {
              await sql`UPDATE accounts SET balance = balance - ${+fee[0].amount}, updated_at = NOW() WHERE unit = ${fee[0].unit} AND account_type = 'Cash'`;
            }
          } catch(e) { console.error('Balance reverse error:', e.message); }
      ...
      case 'salaries': {
        ...
        if (req.method === 'POST') {
          ...
          // AUTO DEDUCT FROM BALANCE - salary payment reduces cash
          if (+amount > 0 && u) {
            try {
              await sql`UPDATE accounts SET balance = GREATEST(0, balance - ${+amount}), updated_at = NOW() WHERE unit = ${u} AND account_type = 'Cash'`;
            } catch(e) { console.error('Balance deduct error:', e.message); }
          }
          return res.status(201).json(r[0]);
        }
        ...
        if (req.method === 'DELETE') {
          if (!id) return res.status(400).json({ error:'id required' });
          // Reverse balance on salary delete
          try {
            const sal = await sql`SELECT * FROM salary_payments WHERE id=${id} LIMIT 1`;
            if (sal.length && +sal[0].amount > 0 && sal[0].unit) {
              await sql`UPDATE accounts SET balance = balance + ${+sal[0].amount}, updated_at = NOW() WHERE unit = ${sal[0].unit} AND account_type = 'Cash'`;
            }
          } catch(e) { console.error('Balance reverse error:', e.message); }
